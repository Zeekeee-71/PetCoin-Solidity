...
;
task("add-liquidity", "Adds liquidity to the CNU/WETH pool")
  .addParam("amountCnu", "Amount of CNU to add", "1000000", types.string) // 1,000,000 CNU
  .addParam("amountWeth", "Amount of WETH to add",              "1", types.string) // 1 WETH
  .setAction(async ({ amountCnu, amountWeth, weth }, hre) => {
    ...
    amountCnu = ethers.parseUnits(amountCnu, 18);
    ...
    await token.approve(router.target, amountCnu);
